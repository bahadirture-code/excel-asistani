...
class CommandEngine:
    ...
    def execute(self, data, df):
        ...
        for step in steps:
            ...
            if action == "filter":
                ...
                result = self.ops.head(
                    result,
                    step.get("count", 10)
                )

            elif action == "tail":
                result = self.ops.tail(
                    result,
                    step.get("count", 10)
                )
                ...
                result = self.ops.top_null_columns(
                    result,
                    step.get("count", 10)
                )
    ...
    def remove_columns(self, df, step):
        ...
        return self.ops.remove_columns(
            df,
            cols
        )
